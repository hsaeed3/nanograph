...
from ._utils.core import nanograph_verbose, get_logger, styles
logger = get_logger()
...
def get_resource() -> _LiteLLMResource:
    ...
    if not _litellm_initialized:
        try:
            ...
            if nanograph_verbose:
                logger.debug(f"{styles.str_bold('LiteLLM')} resource imported & initialized successfully")
    ...
    if nanograph_verbose:
        logger.info(f"{styles.str_bold('LiteLLM')} resource loaded successfully")
